...
// ==================== 知识点库（来自 exam_focus.md） ====================
const KNOWLEDGE_BASE = {
  'Java Web': {
    subject:'Java Web (JSP/Servlet)',
    topics:[
      {id:'jw1',name:'Servlet 生命周期',level:'A',freq:7,content:'init() → service() → destroy()。init只调一次(首次加载)，service每次请求调用，destroy服务器关闭时调用。',keywords:['Servlet生命周期','init','service','destroy']},
      {id:'jw2',name:'JSP 9大隐式对象',level:'A',freq:7,content:'request(HttpServletRequest)、response(HttpServletResponse)、session(HttpSession)、application(ServletContext)、config(ServletConfig)、out(JspWriter)、page(Object)、pageContext(PageContext)、exception(Throwable，仅error页面)。',keywords:['隐式对象','request','response','session','application','out','pageContext']},
      {id:'jw3',name:'请求转发 vs 重定向',level:'A',freq:7,content:'转发(forward)：服务器内部跳转，URL不变，1次请求，可共享request数据，不能跨域。重定向(sendRedirect)：浏览器重新请求，URL改变，2次请求，不能共享数据，可跨域。转发快，重定向慢。',keywords:['转发','重定向','forward','sendRedirect']},
      {id:'jw4',name:'ServletConfig vs ServletContext',level:'A',freq:7,content:'ServletConfig：单个Servlet的初始化参数，仅当前Servlet可用。ServletContext：整个Web应用的参数，所有Servlet共享。获取：getServletConfig().getInitParameter() / getServletContext().getInitParameter()。',keywords:['ServletConfig','ServletContext','初始化参数']},
      {id:'jw5',name:'Filter 过滤器',level:'A',freq:7,content:'实现Filter接口，三个方法：init()→doFilter()→destroy()。FilterChain.doFilter()放行。典型应用：IP访问控制、编码过滤、权限校验。',keywords:['Filter','过滤器','doFilter','FilterChain']},
      {id:'jw6',name:'MVC 开发模式',level:'A',freq:6,content:'Model(模型)：数据存储和业务逻辑。View(视图)：用户交互页面。Controller(控制器)：调度者，接收请求分派给Model处理，再将结果分派给View。典型：login.jsp(View)→LoginServlet(Controller)→LoginDao(Model)。',keywords:['MVC','Model','View','Controller']},
      {id:'jw7',name:'JSP 生命周期/运行流程',level:'A',freq:6,content:'首次请求：.jsp→转译.java(Servlet源码)→编译.class→执行响应。再次请求：直接执行.class。四步：Request→转译→编译加载→Response。',keywords:['JSP生命周期','运行流程','转译','编译']},
      {id:'jw8',name:'Session vs Cookie',level:'A',freq:5,content:'Session存服务器(安全，增服务器压力)，Cookie存浏览器(≤4KB，相对不安全)。登录等重要信息用Session。存Session：request.getSession().setAttribute()；存Cookie：new Cookie() + cookie.setMaxAge() + response.addCookie()。',keywords:['Session','Cookie','会话']},
      {id:'jw9',name:'JSTL流程控制标签',level:'B',freq:4,content:'<c:if>单条件分支; <c:choose>/<c:when>/<c:otherwise>多条件分支; <c:forEach>循环遍历。区别：c:if无else，c:choose支持多分支。',keywords:['JSTL','c:if','c:choose','c:when','c:forEach']},
      {id:'jw10',name:'JSP动作标签',level:'B',freq:4,content:'<jsp:useBean id="user" class="包.User" scope="session"/>创建对象。<jsp:setProperty name="user" property="name" value="lisi"/>设值。<jsp:getProperty name="user" property="name"/>取值。',keywords:['useBean','setProperty','getProperty','动作标签']},
      {id:'jw11',name:'四大作用域',level:'B',freq:4,content:'page(当前页面) < request(一次请求) < session(一次会话) < application(整个应用)。page范围最小，application范围最大。',keywords:['作用域','page','request','session','application']},
      {id:'jw12',name:'JSP指令(page/include/taglib)',level:'B',freq:4,content:'<%@ page import="..." errorPage="..."%>设置页面属性。<%@ include file="..."%>编译时包含。<%@ taglib prefix="c" uri="..."%>引入标签库。',keywords:['page指令','include指令','taglib指令']},
      {id:'jw13',name:'include指令 vs include动作',level:'B',freq:3,content:'指令(<%@ include %>)：编译时合并源码，被包含页修改需重新编译。动作(<jsp:include>)：运行时合并HTML结果，可传参，修改直接生效。',keywords:['include','静态包含','动态包含']},
      {id:'jw14',name:'JDBC开发步骤',level:'B',freq:3,content:'①Class.forName加载驱动 ②DriverManager.getConnection获取连接 ③创建Statement ④执行SQL(executeUpdate/executeQuery) ⑤关闭ResultSet→Statement→Connection。',keywords:['JDBC','Class.forName','DriverManager','Statement','ResultSet']},
      {id:'jw15',name:'web.xml配置Servlet',level:'B',freq:3,content:'一对标签：<servlet>(servlet-name+servlet-class)和<servlet-mapping>(servlet-name+url-pattern)。url-pattern是浏览器访问路径。',keywords:['web.xml','servlet','servlet-mapping','url-pattern']},
      {id:'jw16',name:'EL表达式',level:'B',freq:3,content:'${name}取session属性，${user.name}取JavaBean属性，${map.key}取Map值。比JSP脚本简洁。',keywords:['EL表达式','${}']},
      {id:'jw17',name:'Tomcat配置',level:'B',freq:4,content:'默认端口8080(server.xml)，WEB-INF目录不可直接访问(安全)，work目录存编译后的Servlet类文件。',keywords:['Tomcat','8080','server.xml','WEB-INF']},
      {id:'jw18',name:'编码设置',level:'B',freq:3,content:'request.setCharacterEncoding("UTF-8")、response.setContentType("text/html;charset=UTF-8")、page指令中pageEncoding="UTF-8"。',keywords:['编码','setCharacterEncoding','UTF-8']},
    ]
  },
  'Linux': {
    subject:'Linux 操作系统',
    topics:[
      {id:'lx1',name:'文件/目录操作命令',level:'A',freq:30,content:'ls -a(含隐藏) ls -l(详细信息) cd(切换) pwd(当前路径) mkdir -p(递归创建) rm -r(递归删除) cp(复制) mv(移动/重命名) touch(创建空文件) cat(查看/合并) more/less(分页) head/tail(头尾N行) file(判断类型)。tail -f实时跟踪。',keywords:['ls','cd','pwd','mkdir','rm','cp','mv','touch','cat','tail','head']},
      {id:'lx2',name:'权限管理 chmod',level:'A',freq:15,content:'数字：7=rwx(4+2+1) 6=rw- 5=r-x 4=r--。chmod 764 file→所有者rwx,组rw-,其他r--。符号：chmod u+x 用户加执行，chmod a-x 全部去执行。三组：user/group/others。',keywords:['chmod','权限','rwx','755','764']},
      {id:'lx3',name:'vi/vim 编辑器',level:'A',freq:12,content:'三种模式：命令模式↔插入模式(i/a/o)↔末行模式(:)。保存退出:wq，不保存退出:q!。dd删行，yy复制，p粘贴，nG跳行，nl右移，.重复操作。set nonu取消行号。',keywords:['vi','vim',':wq',':q!','dd','yy','插入模式','命令模式']},
      {id:'lx4',name:'用户管理',level:'A',freq:10,content:'useradd -d /home/xxx 创建用户指定家目录。usermod修改用户(不能删账户！)。userdel -r删除用户及主目录。su切换用户。passwd修改密码。chown改变所有者。',keywords:['useradd','usermod','userdel','su','passwd','chown']},
      {id:'lx5',name:'Shell编程基础',level:'A',freq:15,content:'变量赋值：变量名=值(等号两边无空格！)。引用：$变量名。命令替换：$(命令)。脚本首行：#!/bin/bash。参数个数：$#。注释：#。单引号不解释特殊字符。',keywords:['Shell','#!/bin/bash','变量','$#','export']},
      {id:'lx6',name:'tar压缩/解压',level:'B',freq:8,content:'tar -zcvf 压缩包名 源文件(压缩)。tar -xzvf 压缩包名(解压)。tar -C /path 指定解压目录。tar -t 列出压缩包内容。tar -zcf 等价-zcvf(少v不显示过程)。',keywords:['tar','压缩','解压','-zcvf','-xzvf']},
      {id:'lx7',name:'进程管理',level:'B',freq:8,content:'ps -aux查看全部进程。kill PID终止进程。killall 进程名终止所有同名。top实时监控。PID=进程唯一ID。kill -9强杀。',keywords:['ps','kill','top','PID','进程']},
      {id:'lx8',name:'管道/重定向',level:'B',freq:8,content:'|管道(前命令输出作后命令输入)。>覆盖重定向，>>追加重定向，2>&1错误合并到标准输出。<输入重定向。',keywords:['管道','重定向','|','>','2>&1']},
      {id:'lx9',name:'grep/正则',level:'B',freq:5,content:'grep 字符串 文件(查找匹配行)。支持正则：.任意字符 *零或多次 ^行首 $行尾 []字符集。',keywords:['grep','正则','查找']},
      {id:'lx10',name:'Linux目录结构',level:'B',freq:5,content:'/bin基本命令 /etc配置文件 /home用户主目录 /boot内核 /tmp临时文件 /dev设备 /var日志 /root超级用户家目录。',keywords:['目录结构','/bin','/etc','/home','/boot']},
      {id:'lx11',name:'关机/重启命令',level:'B',freq:4,content:'shutdown -h +30(30分钟后关机) shutdown -h 12:30(定时关机) shutdown -c(取消) halt(立即关机) reboot(重启)。',keywords:['shutdown','halt','reboot']},
      {id:'lx12',name:'软链接 vs 硬链接',level:'B',freq:3,content:'软链接(ln -s)：类似快捷方式，删原文件后失效，可跨文件系统。硬链接(ln)：共享i节点，删原文件仍可用，不能跨文件系统。',keywords:['软链接','硬链接','ln','i节点']},
    ]
  },
  'Hadoop': {
    subject:'Hadoop 大数据技术',
    topics:[
      {id:'hd1',name:'HDFS 架构',level:'A',freq:20,content:'NameNode：管理命名空间和元数据(存"目录"，不存数据)。DataNode：存储实际数据块。SecondaryNameNode：合并fsimage和edits日志(NameNode的"秘书"，不是热备)。',keywords:['HDFS','NameNode','DataNode','SecondaryNameNode','元数据']},
      {id:'hd2',name:'YARN 架构',level:'A',freq:15,content:'ResourceManager(RM)：主节点，全局资源调度。NodeManager(NM)：管理单节点资源。ApplicationMaster(AM)：为应用向RM申请资源。Container：封装CPU+Memory的最小资源单位。全称：Yet Another Resource Negotiator。',keywords:['YARN','ResourceManager','NodeManager','ApplicationMaster','Container']},
      {id:'hd3',name:'Block/副本策略',level:'A',freq:10,content:'默认Block=128MB(HDFS 2.x)。默认副本数=3(dfs.replication)。500MB分4块(128×3+116×1)。副本放置：机架感知，第一个在客户端节点，第二个在不同机架，第三个同机架不同节点。',keywords:['Block','128MB','副本','dfs.replication','机架感知']},
      {id:'hd4',name:'MapReduce流程',level:'A',freq:10,content:'Input→Split→Map→Shuffle→Reduce→Output。Shuffle是Map和Reduce间的关键环节。Map并行度=分片数。默认Reduce数=1。Map输入key=LongWritable(偏移量)，value=Text(行内容)。',keywords:['MapReduce','Split','Map','Shuffle','Reduce']},
      {id:'hd5',name:'HDFS常用命令',level:'A',freq:12,content:'hdfs dfs -mkdir -p(创建多级目录) -put(上传) -get(下载) -ls -R(递归列出) -cat(查看) -rm -r(递归删除) -getmerge(合并下载) -moveFromLocal(移动上传后删本地)。namenode -format格式化。',keywords:['hdfs dfs','-put','-get','-mkdir','-ls','-rm','-cat','-getmerge']},
      {id:'hd6',name:'HDFS HA/Federation',level:'B',freq:5,content:'HA：Active/Standby双NameNode，ZooKeeper实现故障自动切换。Federation：多NameSpace，每个NameNode管理一部分目录。',keywords:['HA','Federation','高可用','双NameNode','ZooKeeper']},
      {id:'hd7',name:'不适合小文件',level:'B',freq:3,content:'原因：NameNode元数据压力大(每个文件一条元数据)，频繁交互降低吞吐，Map任务浪费。解决：文件合并、SequenceFile、HBase、Flume/Spark Streaming聚合。',keywords:['小文件','元数据','SequenceFile','HBase']},
      {id:'hd8',name:'HDFS写数据流程',level:'B',freq:3,content:'Client→NameNode(获取DataNode列表)→向第一个DataNode写数据→DataNode间Pipeline副本传输→写完所有副本返回确认。',keywords:['写数据流程','Pipeline','副本传输']},
    ]
  },
  'Scala & Spark': {
    subject:'Scala & Spark',
    topics:[
      {id:'ss1',name:'Array/ArrayBuffer数组',level:'A',freq:6,content:'Array：定长数组(val arr=Array(1,2,3))，长度不可变但元素可变。ArrayBuffer：变长数组，可动态增删元素（适合频繁增删场景）。数组下标从0开始。',keywords:['Array','ArrayBuffer','数组','定长','变长']},
      {id:'ss2',name:'元组Tuple',level:'A',freq:6,content:'最多22个元素。访问：t._1 t._2 t._3（下标从1开始！不是0）。每个位置可存不同数据类型。创建后不可修改。错误写法：❌ t(0) t[0]。',keywords:['Tuple','元组','._1','._2','22']},
      {id:'ss3',name:'集合List/Set/Map',level:'A',freq:5,content:'List：默认不可变，有序可重复。Set：无序不可重复(去重)。Map：键值对key-value存储。三大核心集合体系。',keywords:['List','Set','Map','集合','不可变']},
      {id:'ss4',name:'var vs val',level:'B',freq:3,content:'var：可重新赋值(变量)。val：不可重新赋值(常量/final)。但val引用的Array内元素可修改(arr(0)=10✅)。',keywords:['var','val','变量','不可变引用']},
      {id:'ss5',name:'def方法定义',level:'B',freq:3,content:'def 方法名(参数:类型): 返回类型 = { ... }。返回值类型在参数列表后面。如def add(a:Int): Int = a+1。',keywords:['def','方法','参数','返回值']},
      {id:'ss6',name:'RDD核心概念',level:'A',freq:5,content:'全称：弹性分布式数据集(Resilient Distributed Dataset)。五大特性：可分区的、可并行计算、容错(血缘)、不可变、可持久化。最大优势vs MapReduce：基于内存计算。不可直接修改数据。',keywords:['RDD','弹性分布式','五大特性','内存计算']},
      {id:'ss7',name:'转换vs行动操作',level:'A',freq:3,content:'转换(Transformation)：惰性求值，不立即执行(map/filter/flatMap)。行动(Action)：触发计算(count/collect/reduce)。',keywords:['转换','行动','Transformation','Action','惰性求值']},
      {id:'ss8',name:'Spark集群模式',level:'B',freq:2,content:'四种运行模式：Standalone(独立集群)、YARN、Mesos、Local(本地模式)。',keywords:['Standalone','YARN','Mesos','Local','集群模式']},
    ]
  },
  'HBase / ZooKeeper': {
    subject:'HBase / ZooKeeper / 大数据概念',
    topics:[
      {id:'hz1',name:'HBase架构',level:'A',freq:6,content:'HMaster：主节点，Region分配/负载均衡/状态监控。RegionServer：从节点，处理客户端读写请求。ZooKeeper：HMaster选举、元数据存储、集群协调。Client：与RegionServer直接交互。底层存储HDFS。',keywords:['HMaster','RegionServer','Region','HDFS']},
      {id:'hz2',name:'ZooKeeper机制',level:'A',freq:6,content:'集群2n+1台(奇数)，允许n台失效。ZNode最大1MB。节点类型：持久/临时/持久顺序/临时顺序。选举：过半机制(超半数同意选出Leader)。myid文件标识节点。ls查看子节点。',keywords:['ZooKeeper','ZNode','过半机制','选举','myid']},
      {id:'hz3',name:'HBase数据模型',level:'A',freq:4,content:'RowKey(行键，字典序排序)、Column Family(列族)、Timestamp(时间戳标识版本)、Cell(单元格=RowKey+CF+Qualifier+Timestamp)。NameSpace命名空间默认default。',keywords:['RowKey','Column Family','Timestamp','Cell','NameSpace']},
      {id:'hz4',name:'HBase特点',level:'B',freq:3,content:'列式存储、稀疏性(空列不占空间)、易扩展、海量存储(PB级)、高并发读写。Apache顶级项目2010年。',keywords:['列式存储','稀疏性','高并发','PB级']},
      {id:'hz5',name:'HBase配置',level:'B',freq:4,content:'依赖HDFS+ZK。HBASE_MANAGES_ZK=false使用外部ZK。hbase.cluster.distributed=true。环境变量配/etc/profile。source使配置生效。Master Web默认端口16010(1.0+)。',keywords:['HBASE_MANAGES_ZK','hbase.cluster.distributed','16010','/etc/profile']},
      {id:'hz6',name:'大数据4V特点',level:'C',freq:2,content:'Volume(大量)、Velocity(高速)、Variety(多样)、Value(价值密度低⭕不是高❌)。',keywords:['大数据','4V','Volume','Velocity','Variety','Value']},
    ]
  },
  'Web前端': {
    subject:'Web前端 (HTML/CSS/JS)',
    topics:[
      {id:'wf1',name:'HTML基础结构',level:'A',freq:5,content:'<html><head>...</head><body>...</body></html>。head放元数据(title/meta/link/style)，body放可见内容。这是网页最基础框架。',keywords:['HTML','html','head','body','结构']},
      ...
      {id:'wf3',name:'HTML表格',level:'B',freq:4,content:'<table>表格，<tr>行，<td>单元格，<th>表头(粗体居中)。属性：border边框粗细，cellspacing单元格间距，cellpadding单元格内边距，align对齐方式。<th>必须放在<tr>中。',keywords:['table','tr','td','th','表格','border','cellspacing','cellpadding']},
      {id:'wf4',name:'HTML表单',level:'A',freq:6,content:'<form>表单，<input>输入框。type类型：text文本、password密码、hidden隐藏、submit提交、radio单选、checkbox多选、email邮箱。<textarea>多行文本。<select>下拉。name属性对应后台参数名。',keywords:['form','input','text','password','submit','表单','textarea','select']},
      {id:'wf5',name:'CSS选择器',level:'A',freq:8,content:'#id选择器(唯一，权重100)；.class类选择器(可复用，权重10)；标签选择器(权重1)；*通配符(权重0)；后代选择器(div p)；群组选择器(a,b)。优先级：!important > 行内 > id > class > 标签 > *。',keywords:['CSS','选择器','id','class','优先级','权重','#','.']},
      {id:'wf6',name:'CSS盒模型',level:'A',freq:8,content:'四部分：content(内容)+padding(内边距)+border(边框)+margin(外边距)。box-sizing:content-box(默认)→width仅内容，总宽=width+padding+border。box-sizing:border-box→width含padding+border，总宽=width。border-box更直观。',keywords:['盒模型','content','padding','border','margin','box-sizing','content-box','border-box']},
      {id:'wf7',name:'CSS定位(position)',level:'A',freq:5,content:'static：默认，正常文档流。relative：相对自身原位置偏移，仍占空间。absolute：相对最近定位祖先/窗口，脱离文档流。fixed：固定视口位置，脱离文档流。sticky：粘性定位(relative+fixed结合)。',keywords:['position','static','relative','absolute','fixed','定位','sticky']},
      {id:'wf8',name:'CSS居中方式',level:'A',freq:4,content:'块级水平居中：margin:0 auto(必须有宽度)。行内/文本居中：text-align:center。垂直居中：flexbox(display:flex+align-items:center)、line-height=height、绝对定位+transform。',keywords:['居中','margin','text-align','flex','水平居中','垂直居中']},
      {id:'wf9',name:'display属性',level:'A',freq:5,content:'block：块级元素(独占一行，可设宽高)。inline：行内元素(不换行，宽高无效)。inline-block：行内块(不换行，可设宽高)。none：隐藏且不占空间。flex/grid：弹性/网格布局。visibility:hidden：隐藏但占空间。',keywords:['display','block','inline','inline-block','none','visibility','flex','grid']},
      {id:'wf10',name:'CSS常用属性',level:'A',freq:6,content:'文字：font-size字号、font-family字体族、font-weight粗细、color颜色。背景：background-color、background-image(url)。边框：border:粗细 样式 颜色。文本：text-align对齐、line-height行高、text-decoration装饰。',keywords:['font-size','font-family','font-weight','color','background','border','text-align','line-height']},
      {id:'wf11',name:'JavaScript变量声明',level:'A',freq:5,content:'var：函数作用域，存在变量提升，可重复声明。let：块级作用域，存在TDZ(暂时性死区)，不可重复声明。const：块级作用域，不可重新赋值，但对象属性可修改。推荐优先const，需要改再let。',keywords:['var','let','const','变量','作用域','提升','TDZ']},
      {id:'wf12',name:'JavaScript数据类型',level:'A',freq:4,content:'基本类型：number/string/boolean/null/undefined/symbol/bigint。引用类型：object(array/function/date)。typeof检测：typeof null→"object"(历史bug)。null==undefined→true，null===undefined→false。NaN不等于自身。',keywords:['数据类型','null','undefined','typeof','NaN','number','string','boolean']},
      {id:'wf13',name:'DOM操作',level:'A',freq:6,content:'获取元素：getElementById(id)→单个元素(找不到返回null)；getElementsByClassName/ByTagName→HTMLCollection(动态)；querySelector(css选择器)→单个；querySelectorAll→NodeList(静态)。innerHTML获取/设置内容。',keywords:['DOM','getElementById','querySelector','querySelectorAll','innerHTML','getElementsByClassName']},
      {id:'wf14',name:'JavaScript事件',level:'B',freq:4,content:'常用事件：onclick点击、onsubmit表单提交、onload页面加载、onchange值改变、onmouseover/out悬停。事件绑定：元素.onclick=fn 或 addEventListener("click",fn)。form验证常用onsubmit="return check()"。',keywords:['事件','onclick','onsubmit','onload','addEventListener','event']},
      {id:'wf15',name:'CSS引入方式',level:'B',freq:3,content:'三种方式：1.行内样式(<p style="...">)优先级最高；2.内嵌样式(<head>中<style>标签)优先级中等；3.外部样式(<link rel="stylesheet" href=".css">)优先级最低。优先级：行内>内嵌>外部。!important可打破规则。',keywords:['CSS引入','行内样式','内嵌样式','外部样式','link','style','优先级']},
      {id:'wf16',name:'HTML5语义化标签',level:'B',freq:3,content:'<header>页头、<nav>导航、<main>主体、<article>独立内容、<section>分节、<aside>侧栏、<footer>页脚。语义化好处：SEO友好、可访问性高、代码可读性强。替代无意义的<div>。',keywords:['HTML5','语义化','header','nav','main','article','section','footer','aside']},
      {id:'wf17',name:'CSS浮动(float)',level:'B',freq:3,content:'float:left/right让元素浮动，文字环绕。浮动元素脱离文档流，父容器高度塌陷。清除浮动：clear:both、overflow:hidden、::after伪元素清除法(clearfix)。现代布局推荐flexbox替代。',keywords:['float','浮动','clear','清除浮动','clearfix']},
      {id:'wf18',name:'CSS Flexbox布局',level:'B',freq:3,content:'display:flex开启弹性布局。容器属性：justify-content主轴对齐、align-items交叉轴对齐、flex-direction方向、flex-wrap换行。项目属性：flex:占比例、align-self单独对齐。一维布局首选方案。',keywords:['flex','flexbox','justify-content','align-items','弹性布局']},
      {id:'wf19',name:'响应式与媒体查询',level:'C',freq:2,content:'@media screen and (max-width:768px){...}根据屏幕宽度应用不同CSS。移动优先：先写移动端样式再用@media适配桌面。viewport：<meta name="viewport" content="width=device-width">。',keywords:['响应式','媒体查询','@media','viewport','移动端','自适应']},
      {id:'wf20',name:'表单验证',level:'B',freq:3,content:'HTML5原生验证：required必填、pattern正则、type="email"邮箱格式、min/max范围。JavaScript验证：onsubmit事件中检查，不通过return false阻止提交。alert()提示用户错误信息。document.formName.fieldName.value获取表单值。',keywords:['表单验证','required','pattern','onsubmit','return false','alert']},
    ]
  }
};
