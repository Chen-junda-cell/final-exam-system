...
const KNOWLEDGE_BASE = {
  'Java Web': {
    ...
    topics:[
      ...
      {id:'jw2',name:'JSP 9大隐式对象',level:'A',freq:9,content:'request(请求)、response(响应)、session(会话)、application(全局)、config(配置)、out(输出)、page(当前页)、pageContext(页面上下文)、exception(异常,仅错误页)。四大作用域从小到大：page<request<session<application。',keywords:['隐式对象','request','response','session','application','out','pageContext','作用域']},
      {id:'jw3',name:'请求转发 vs 重定向',level:'A',freq:8,content:'转发(forward)：服务器内部跳转、URL不变、1次请求、可共享request数据、不能跨域、速度快。代码：request.getRequestDispatcher("url").forward(req,resp)。重定向(sendRedirect)：浏览器新请求、URL改变、2次请求、不能共享数据、可跨域。代码：response.sendRedirect("url")。',keywords:['转发','重定向','forward','sendRedirect','RequestDispatcher']},
      {id:'jw4',name:'Servlet 生命周期',level:'A',freq:8,content:'三阶段：init()初始化(首次加载调1次)→service()/doGet()/doPost()每次请求调用→destroy()服务器关闭时调用1次。Servlet是单例多线程的，每个请求分配一个线程调用service方法。',keywords:['Servlet生命周期','init','service','destroy','doGet','doPost']},
      ...
      {id:'jw10',name:'JSP 三大指令',level:'A',freq:6,content:'page指令：<%@ page import="包" pageEncoding="UTF-8" errorPage="错误页"%>设置页面属性。include指令：<%@ include file="文件"%>编译时静态包含。taglib指令：<%@ taglib prefix="c" uri="..."%>引入JSTL等标签库。',keywords:['page指令','include指令','taglib指令','<%@']},
  ...
  'Linux': {
    subject:'Linux 操作系统',
    topics:[
      {id:'lx1',name:'文件/目录操作命令大全',level:'A',freq:15,content:'ls -a(含隐藏) -l(详细信息) -d(目录本身)；cd ~(回家) ..(上级) /(根)；pwd(当前路径)；mkdir -p(递归创建)；rmdir(删空目录)；rm -r(递归删) -f(强制)；cp -r(复制目录)；mv(移动/重命名)；touch(建空文件)；cat(查看/合并)；more/less(分页)；head -n/tail -n(头尾N行)；tail -f(实时追踪)。',keywords:['ls','cd','pwd','mkdir','rm','cp','mv','touch','cat','tail','head','less']},
      ...
      {id:'lx5',name:'Shell 脚本编程',level:'A',freq:10,content:'首行必须写#!/bin/bash。变量赋值等号两边不能有空格：name="value"。引用变量：$name或${name}。命令替换：$(命令)或反引号。参数：$#(个数) $0(脚本名) $1 $2(第n个参数)。条件：if [ 条件 ]; then ... fi。循环：for i in ...; do ... done。',keywords:['Shell','#!/bin/bash','变量','$#','$?','if','for','while','case']},
      ...
      {id:'lx13',name:'Linux 特点与发行版',level:'C',freq:3,content:'7大特点：与UNIX兼容、自由软件源码公开、性能高安全性强、便于定制再开发、互操作性高、真正的多任务32/64位、服务器+嵌入式领域应用广泛。主流发行版CentOS 7(考试默认)。',keywords:['Linux特点','UNIX','开源','CentOS','发行版']},
    ]
  },
  'Hadoop': {
    subject:'Hadoop 大数据技术',
    topics:[
      {id:'hd1',name:'HDFS 核心架构',level:'A',freq:12,content:'NameNode(主节点)：管理元数据和命名空间(存"目录"，不存实际数据)。DataNode(从节点)：存储实际数据块，定期向NameNode发送心跳。SecondaryNameNode：合并fsimage和edits日志，不是热备！',keywords:['HDFS','NameNode','DataNode','SecondaryNameNode','元数据','心跳']},
      {id:'hd2',name:'YARN 资源管理架构',level:'A',freq:10,content:'ResourceManager(RM)：全局资源调度主节点。NodeManager(NM)：单节点资源管理，向RM汇报。ApplicationMaster(AM)：为每个应用向RM申请资源并监控任务。Container：封装CPU+Memory的最小资源抽象单元。YARN全称：Yet Another Resource Negotiator。',keywords:['YARN','ResourceManager','NodeManager','ApplicationMaster','Container']},
      ...
      {id:'hd5',name:'MapReduce 核心流程',level:'A',freq:10,content:'Input→Split(分片)→Map(映射转换)→Shuffle(分区排序归并)→Reduce(聚合汇总)→Output。Map并行度=Split数量。默认Reduce数=1。Map输入key=LongWritable(字节偏移量)，value=Text(行内容)。Shuffle是MapReduce的核心环节。',keywords:['MapReduce','Split','Map','Shuffle','Reduce','InputFormat']},
  ...
  'Scala & Spark': {
    subject:'Scala & Spark',
    topics:[
      {id:'ss1',name:'var vs val 变量定义',level:'A',freq:6,content:'var：变量，可重新赋值。val：常量(不可变引用)，类似Java final。但val引用的Array/ArrayBuffer内元素可修改。Scala推荐优先用val(函数式编程风格)。代码：var x=10; x=20 ✅ | val y=10; y=20 ❌编译错误。',keywords:['var','val','变量','常量','不可变']},
      ...
      {id:'ss5',name:'RDD 核心概念与五大属性',level:'A',freq:6,content:'RDD=弹性分布式数据集(Resilient Distributed Dataset)。五大属性：①分区列表②每分区计算函数③依赖关系(宽/窄)④分区器(可选,KV类型)⑤优先位置列表。不可变、可分区、可并行计算、基于内存。',keywords:['RDD','弹性分布式','五大属性','分区','依赖','不可变','内存计算']},
      ...
      {id:'ss10',name:'Spark 运行模式',level:'B',freq:4,content:'四种模式：Local(本地开发，local[*]用所有核)、Standalone(自带集群)、YARN(Hadoop资源管理，生产常用)、Mesos/K8s。设置方式：setMaster("local[*]")或SparkSession.builder().master("yarn")。',keywords:['Local','Standalone','YARN','Mesos','K8s','setMaster']},
      ...
      {id:'ss12',name:'Spark 生态组件与选型',level:'B',freq:5,content:'Spark Core(核心引擎)、Spark SQL(结构化/SQL)、Spark Streaming(流式实时)、MLlib(机器学习/推荐)、GraphX(图计算/社交网络)。场景选组件：SQL/报表→Spark SQL；实时告警→Streaming；推荐模型→MLlib；社交关系→GraphX。',keywords:['Spark Core','Spark SQL','Spark Streaming','MLlib','GraphX','组件','选型']},
      {id:'ss13',name:'Scala 面向对象',level:'B',freq:4,content:'class：多例(可new多个实例)。object：单例(全局唯一，类似Java static)。extends：继承。override：重写(必须写！)。trait：特质(类似Java8接口，可多重混入with)。abstract class：抽象类(不能实例化)。主构造器执行类体中的所有代码。',keywords:['class','object','extends','override','trait','abstract','单例','伴生对象']},
  ...
  'HBase / ZooKeeper': {
    ...
    topics:[
      {id:'hz1',name:'HBase 核心架构',level:'A',freq:8,content:'HMaster：主节点，Region分配/负载均衡/状态监控。RegionServer：从节点，处理客户端读写请求，管理Region。ZooKeeper：HMaster选举、元数据存储(hbase:meta表位置)、集群协调。Client直接与RegionServer交互。底层存储依赖HDFS。',keywords:['HMaster','RegionServer','Region','ZooKeeper','HDFS','HBase']},
      ...
      {id:'hz3',name:'HBase 数据模型',level:'A',freq:7,content:'RowKey(行键，字典序排序，设计最关键！)、Column Family(列族，创建表时定义)、Timestamp(时间戳，标识数据版本)、Cell(单元格=RowKey+CF+列限定符+时间戳)。NameSpace(命名空间，默认default)。表水平切分为多个Region。',keywords:['RowKey','Column Family','Timestamp','Cell','NameSpace','Region','列族']},
      {id:'hz4',name:'HBase 核心特点',level:'A',freq:6,content:'①列式存储；②稀疏性(同一表不同行可有不同列，空列不占存储空间)；③易扩展(加节点即可)；④海量存储(PB级)；⑤高并发随机读写(实时查询)；⑥面向列族设计。底层基于HDFS，参考Google Bigtable设计。全称：Hadoop Database。',keywords:['HBase特点','列式存储','稀疏性','高并发','PB级','Bigtable']},
      {id:'hz5',name:'HBase Shell 命令',level:'B',freq:6,content:'create 表名,列族名(建表)；put 表名,rowkey,列族:列,值(插入)；get 表名,rowkey(获取单行)；scan 表名(扫描全表)；delete(删除)；disable+drop(删表)；describe(查看表结构)；list(列出所有表)；count(统计行数)；truncate(清空表)。',keywords:['HBase Shell','create','put','get','scan','disable','drop','list']},
      ...
      {id:'hz7',name:'大数据 4V 特点',level:'B',freq:4,content:'Volume(大量)：数据量巨大PB/ZB级。Velocity(高速)：数据产生和处理速度快。Variety(多样)：结构化+半结构化+非结构化数据。Value(价值密度低)：海量数据中有价值的信息比例低(不是高！)。这是大数据的经典定义。',keywords:['大数据','4V','Volume','Velocity','Variety','Value','价值密度低']},
  ...
  'Web前端': {
    ...
    topics:[
      ...
      {id:'wf9',name:'display 属性',level:'A',freq:5,content:'block：块级独占一行可设宽高。inline：行内不换行宽高无效。inline-block：不换行可设宽高。none：隐藏不占空间。flex/grid：弹性/网格布局。',keywords:['display','block','inline','inline-block','none','flex','grid']},
